...
bool World::handleEscapeSequence(int* pos, unsigned char* ch, int* color) {
	assert(_data[*pos] == 27);
	++(*pos);
	...
	assert(_data[*pos] == ';');
	++(*pos);
	*color = 10 * (_data[*pos] - '0') + (_data[*pos + 1] - '0') + offset;
	*pos += 2;
...
void World::update() {
	...
	unsigned char ch = 0; // color of the char
	...
	Debug::warning() << "_data_size " << _data_size << std::endl;
	...
	for (int pos = 0; pos < _data_size;) {
		...
		bool tostart = handleEscapeSequence(&pos, &ch, &charcolor);
		if (tostart) {
			_cursor.row(0);
			_cursor.col(0);
		}
		...
		assert (_cursor.col() < COLS && _cursor.row() < ROWS && _cursor.col() >= 0 && _cursor.row() >= 0);
		_view[_cursor.row()][_cursor.col()] = ch;
	...
	_cursor = agent;
	_cursor.row(_cursor.row() + 1);
	//_cursor.col(_cursor.col() - 1);
	...
	if (!_menu && !_question && (!parsed_attributes || !parsed_status || _cursor.row() < MAP_ROW_BEGIN || _cursor.row() > MAP_ROW_END || _cursor.col() < MAP_COL_BEGIN || _cursor.col() > MAP_COL_END)) {
		...
		Debug::warning() << "CURSOR ON UNEXPECTED LOCATION: " << parsed_attributes << " " << parsed_status << std::endl;
